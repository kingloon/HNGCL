...
def generate_feature_graph_edge_index(x, k):
    x = F.normalize(x)
    sim = torch.mm(x, x.t())
    sim_index = sim.argsort(descending=True, dim = 1)
    num_nodes = sim_index.size()[0]
    edge_index = np.zeros((2, num_nodes * k), dtype=np.int64)
    idx = 0
    for i in range(num_nodes):
        for j in range(1, k + 1):
            edge_index[0, idx] = i
            edge_index[1, idx] = sim_index[i, j]
            idx = idx + 1
